...
parser.add_argument('--prompts', type=str, required=True,
                                        help='file containing prompts to feed the model with')
parser.add_argument('--control_code', type=str, required=True,
                                        help='control code to attach to every prompt')
...
prompts_file = open(args.prompts, 'r')
prompts_string = prompts_file.read()
prompts_file.close()

prompts = prompts_string.split('\n')
prompts = [args.control_code + ' ' + prompt for prompt in prompts] 

tot_prompts = len(prompts)
...
for i,prompt in enumerate(prompts):
    ...
    print("-----------Starting caption {} of {}-----------".format(i, tot_prompts))
    ...
    if not any(split_prompt[0] == x for x in [CONTROL_CODES.keys(), "caption", "formal", "informal"]):
    ...
    try:
        ...
        # write caption generated on file
        out.write(tokens_generated_so_far + '\n')
